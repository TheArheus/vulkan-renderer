...
GetPhysicalDeviceSurfaceFormat(VkPhysicalDevice Device, VkSurfaceKHR Surface)
{
    ...
    for(uint32_t FormatIndex = 0;
        ...
        ++FormatIndex)
    {
        if(SurfaceFormats[FormatIndex].format == VK_FORMAT_A2B10G10R10_UNORM_PACK32 || SurfaceFormats[FormatIndex].format == VK_FORMAT_A2R10G10B10_UNORM_PACK32)
...
static VkDevice
CreateDevice(VkInstance Instance, VkPhysicalDevice PhysicalDevice, uint32_t* FamilyIndex)
{
    ...
    PhysicalDeviceCreateInfo.ppEnabledExtensionNames = Extensions;
    PhysicalDeviceCreateInfo.enabledExtensionCount = ArraySize(Extensions);
    //PhysicalDeviceCreateInfo.pEnabledFeatures = &Features.features;
...
CreateSwapchain(VkPhysicalDevice PhysicalDevice, VkDevice Device, VkSurfaceKHR Surface, main_state* MainState, uint32_t* FamilyIndex, VkFormat Format)
{
    ...
    SwapchainCreateInfo.minImageCount         = Max(2, SurfaceCaps.minImageCount);
...
{
#if RTX
    VkDescriptorSetLayoutBinding BindingsSet[2] = {};
    BindingsSet[0].binding = 0;
    BindingsSet[0].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    BindingsSet[0].descriptorCount = 1;
    BindingsSet[0].stageFlags = VK_SHADER_STAGE_MESH_BIT_NV;

    BindingsSet[1].binding = 1;
    BindingsSet[1].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    BindingsSet[1].descriptorCount = 1;
    BindingsSet[1].stageFlags = VK_SHADER_STAGE_MESH_BIT_NV;
#else
    VkDescriptorSetLayoutBinding BindingsSet[1] = {};
    BindingsSet[0].binding = 0;
    BindingsSet[0].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    BindingsSet[0].descriptorCount = 1;
    BindingsSet[0].stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
#endif

    VkDescriptorSetLayoutCreateInfo SetCreateInfo = {VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO};
    SetCreateInfo.flags = VK_DESCRIPTOR_SET_LAYOUT_CREATE_PUSH_DESCRIPTOR_BIT_KHR;
    SetCreateInfo.bindingCount = ArraySize(BindingsSet);
    SetCreateInfo.pBindings = BindingsSet;
...
VkPipeline
CreateGraphicsPipeline(VkDevice Device, VkRenderPass RenderPass, VkPipelineCache PipelineCache, VkShaderModule TriangleVert, VkShaderModule TriangleFrag, VkPipelineLayout PipelineLayout)
{
...
#endif
    Stages[0].module = TriangleVert;
    Stages[0].pName  = "main";

    Stages[1].sType  = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    Stages[1].stage  = VK_SHADER_STAGE_FRAGMENT_BIT;
    Stages[1].module = TriangleFrag;
    Stages[1].pName  = "main";

    VkPipelineVertexInputStateCreateInfo VertexInput = {VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO};

#if 0
    // Not long term correct path
    VkVertexInputBindingDescription Stream[1] = {};
    Stream[0] = {0, sizeof(vertex), VK_VERTEX_INPUT_RATE_VERTEX};
    ...
    VertexInput.vertexBindingDescriptionCount = ArraySize(Stream);
    VertexInput.pVertexBindingDescriptions = Stream;
    VertexInput.vertexAttributeDescriptionCount = ArraySize(AttrDesc);
...
void
CreateBuffer(buffer& Result, VkDevice Device, VkPhysicalDeviceMemoryProperties* MemProperty, size_t Size, VkBufferUsageFlags Flags)
{
    ...
    uint32_t MemoryType = SelectMemoryType(*MemProperty, MemoryR.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
...
int main(int argc, char** argv)
{
...
#if RTX
    VkShaderModule TriangleVert = LoadShader(Device, "..\\shaders\\meshlet.mesh.spv");
    assert(TriangleVert);
#else
    VkShaderModule TriangleVert = LoadShader(Device, "..\\shaders\\triangle.vert.spv");
    assert(TriangleVert);
#endif
    VkShaderModule TriangleFrag = LoadShader(Device, "..\\shaders\\triangle.frag.spv");
    assert(TriangleFrag);
    ...
    VkPipeline TrianglePipeline = CreateGraphicsPipeline(Device, RenderPass, PipelineCache, TriangleVert, TriangleFrag, PipelineLayout);
    ...
    //LoadMesh("../data/cube.obj");
    //LoadMesh("../data/f22.obj");
    LoadMesh("../data/kitten.obj");

    buffer VertexBuffer = {};
    uint32_t VertexBufferSize = 128 * 1024 * 1024; //sizeof(v3) * Meshes[0].Vertices.size();
    CreateBuffer(VertexBuffer, Device, &MemProperty, VertexBufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);

    buffer IndexBuffer  = {};
    uint32_t IndexBufferSize = 128 * 1024 * 1024; //sizeof(face_t) * Meshes[0].Indices.size();
    CreateBuffer(IndexBuffer,  Device, &MemProperty, IndexBufferSize, VK_BUFFER_USAGE_INDEX_BUFFER_BIT);

#if RTX
    buffer MeshletBuffer  = {};
    uint32_t MeshletBufferSize = 128 * 1024 * 1024; //sizeof(face_t) * Meshes[0].Indices.size();
    CreateBuffer(MeshletBuffer,  Device, &MemProperty, MeshletBufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);

    assert(Meshes[0].Meshlets.size() * sizeof(uint32_t) <= IndexBuffer.Size);
    memcpy(MeshletBuffer.Data, Meshes[0].Meshlets.data(), Meshes[0].Meshlets.size() * sizeof(uint32_t));
#endif

    assert(Meshes[0].Mesh.size() * sizeof(vertex) <= VertexBuffer.Size);
    memcpy(VertexBuffer.Data, Meshes[0].Mesh.data(), Meshes[0].Mesh.size() * sizeof(vertex));

    assert(Meshes[0].ConvertedVertexIndices.size() * sizeof(uint32_t) <= IndexBuffer.Size);
    memcpy(IndexBuffer.Data, Meshes[0].ConvertedVertexIndices.data(), Meshes[0].ConvertedVertexIndices.size() * sizeof(uint32_t));
...
#if RTX
    PFN_vkCmdDrawMeshTasksNV vkCmdDrawMeshTasksNV = (PFN_vkCmdDrawMeshTasksNV)vkGetInstanceProcAddr(Instance, "vkCmdDrawMeshTasksNV");
#endif
    ...
    {
        double FrameBegin = SDL_GetPerformanceCounter();
        ...
        // NOTE: GPU Frame count is weird, I'll try to fix this
        ...
        double cpu_ms = (FrameEnd - FrameBegin) * 1000.0f / (double)SDL_GetPerformanceFrequency();
